    ...
    \item
          ...
          \begin{enumerate}[label=\alph*)]
              ...
              \item \textit{Without using any neural network toolkit, implement a multi-layer
                    ...
                    \begin{figure}[H]
                        \centering
                        \includesvg[width=0.75\textwidth]{./1/report/assets/q1-mlp-loss-20-0.001.svg}
...
\begin{enumerate}[leftmargin=\labelsep]
    \item \textit{Implement a linear model with logistic regression, using stochastic gradient descent as your training algorithm (use a batch size of 16). Train your model for 20 epochs and tune the learning rate on your validation data, using the following values: $\{0.001, 0.01, 0.1\}$.  Report the best configuration (in terms of final validation accuracy) and plot two things: the training loss and the validation accuracy, both as a function of the epoch number. Report the final accuracy on the test set.}
          ...
          \begin{figure}[H]
              \centering
              \includesvg[width=0.7\textwidth]{./1/report/assets/q2-logistic_regression-validation-accuracy-batch-16-lr-0.01-epochs-20-l2-0.0-opt-sgd}
    ...
    \item \textit{In this exercise, you will design a multilayer perceptron to compute a Boolean function of $D$ variables, $f; \{-1, +1\}^D \mapsto \{-1, +1\}$, defined as:}
          ...
          \begin{enumerate}[label=\alph*)]
              ...
              \item \textit{Show that the function above can be computed with a multilayer perceptron with a single hidden layer with two hidden units and hard threshold activations $g: \mathbb{R} \rightarrow \{-1, +1\}$ with}
                    ...
                    \begin{figure}[H]
                        \centering
                        \includesvg[width=0.75\textwidth]{./1/report/assets/xor-separated.svg}
                    ...
                    \begin{equation}
                        \sum\limits_{i=1}^D x_i = A
                    \end{equation}

                    \begin{equation}
                        \sum\limits_{i=1}^D x_i = B
                    \end{equation}
                    ...
                    \begin{equation}
                        \begin{gathered}
                            W^{(1)} = \begin{bmatrix}
                                1 & \dots & 1 \\
                                1 & \dots & 1
                            \end{bmatrix} \text{ with dimensions }(2 \times D) \\
                            b^{(1)} = \begin{bmatrix}
                                B \\
                                A
                            \end{bmatrix}                                     \\
              ...
              \item \textit{ Repeat the previous exercise if the hidden units use rectified linear unit activations instead.}
